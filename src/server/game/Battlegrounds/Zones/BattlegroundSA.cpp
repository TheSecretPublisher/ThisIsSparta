...
void BattlegroundSA::AddPlayer(Player* player)
{
    ...
    /*if (!ShipsStarted)
    {
        if (player->GetTeamId() == Attackers)
        {
            ...
            if (urand(0, 1))
                player->TeleportTo(607, 2675.968f, -824.614f, 15.0f, 3.548f, 0);
            else
                player->TeleportTo(607, 2575.461f, 980.261f, 15.0f, 2.904f, 0);
        }
        else
    ...
    else*/
    {
        if (player->GetTeamId() == Attackers)
        {
            if (urand(0, 1))
                ...
                player->TeleportTo(607, 1607.975f, 49.271f, 7.579f, 2.36f, 0);
        }
        else
            player->TeleportTo(607, 1209.7f, -65.16f, 70.1f, 0.0f, 0);
    }
...
void BattlegroundSA::GunStartState(bool start)
{
    if (!BgCreatures[0])
        return;
...
void BattlegroundSA::BombStartState(bool start)
{
    if (!BgObjects[0])
        return;
    ...
    for (uint8 i = BG_SA_BOMB; i < BG_SA_MAXOBJ; i++)
    {
        if (Object* bomb = GetBGObject(i))
        {
            if (start)
            {
                sLog->outError(LOG_FILTER_GENERAL, "NOT SELECTABLE");
                bomb->SetFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
            }
            else
                bomb->RemoveFlag(GAMEOBJECT_FLAGS, GO_FLAG_NOT_SELECTABLE);
        }
    }
}
