...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage* damageInfo, int32 damage, SpellInfo const* spellInfo, WeaponAttackType attackType, bool crit)
{
    ...
    switch (spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MELEE:
        {
            ...
            // Scourge Strike shadow damage is not reduced by resilience
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    if (sSpellMgr->GetSpellInfo(trigger_spell_id) == NULL)
    {
        switch (auraSpellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
                ...
                {
                    case 99252:
                        if (this->GetMapId() != 720 && GetAura(99252))
                            GetAura(99252)->Remove();
                        break;
                    case 86303:    
    ...
    switch (auraSpellInfo->Id)
    {
        ...
        case 49571:
        case 49572:
            // Can't proc while in arena preparation or while having Dark Trasformation
            if (this->HasAura(32727) || victim->HasAura(63560))
                return false;

            if(Pet* ghoul = this->ToPlayer()->GetPet())
                if(ghoul->HasAura(63560))
                    return false;
        ...
        case SPELLFAMILY_PALADIN:
            switch(spellProto->Id)
            {
                ...
                case 879:
                    ...
                    {
                        if(this->GetTotalAttackPowerValue(BASE_ATTACK) > player->SpellBaseDamageBonusDone(spellProto->GetSchoolMask()))
                            DoneTotal += this->GetTotalAttackPowerValue(BASE_ATTACK) * 0.0688f;
                        else
                            DoneTotal += player->SpellBaseDamageBonusDone(spellProto->GetSchoolMask()) * 0.0688f;
                    }
                break;
                // Consecration
                case 26573:
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack) const
{
    ...
    for (AuraEffectList::const_iterator i = mHealingDonePctOnDebuff.begin(); i != mHealingDonePctOnDebuff.end(); ++i)
    {
        if (victim)
        {
            // Find a general way to retrive aura to check [in dbc only 99252 it's used atm]
            if (victim->HasAura(99252))
                AddPct(DoneTotalMod, (*i)->GetAmount());
        }
    }
    ...
    switch (spellProto->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
            switch(spellProto->Id)
            {
                // Healing Death Coil
                case 47633:
                    healamount += CalculatePct(this->GetTotalAttackPowerValue(BASE_ATTACK), 23);
                    AddPct(DoneTotalMod, 350);
                    break;
            }
            break;
        case SPELLFAMILY_PALADIN:
...
bool Unit::IsTriggeredAtSpellProcEvent(Unit* victim, Aura* aura, SpellInfo const* procSpell, uint32 procFlag, uint32 procExtra, WeaponAttackType attType, bool isVictim, bool active, SpellProcEventEntry const* & spellProcEvent)
{
    ...
    if (EventProcFlag & PROC_FLAG_KILL && GetTypeId() == TYPEID_PLAYER)
    {
		...
		if (victim && victim->GetEntry() == 19668 || victim->GetEntry() == 46956)
            allow = false;
