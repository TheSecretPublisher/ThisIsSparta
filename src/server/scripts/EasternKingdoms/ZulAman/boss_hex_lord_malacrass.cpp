                    ...
                    switch (eventId)
                    {
                        ...
                        case EVENT_DK_DEATH_AND_DECAY:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_DK_DEATH_AND_DECAY, false);
                            events.ScheduleEvent(EVENT_DK_DEATH_AND_DECAY, urand(10000, 12000));
                            break;
                        case EVENT_DK_BLOOD_WORMS:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_DK_BLOOD_WORMS, false);
                            break;
                        case EVENT_DR_TYPHOON:
                            me->CastSpell(me->getVictim(), SPELL_DR_TYPHOON, false);
                            events.ScheduleEvent(EVENT_DR_TYPHOON, 10000);
                            break;
                        case EVENT_DR_MOONFIRE:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_DR_MOONFIRE, false);
                        ...
                        case EVENT_MG_FROSTBOLT:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_MG_FROSTBOLT, false);
                            events.ScheduleEvent(EVENT_MG_FROSTBOLT, 2000);
                            break;
                        case EVENT_MG_ICE_LANCE:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_MG_ICE_LANCE, false);
                        ...
                        case EVENT_PR_SW_PAIN:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_PR_SW_PAIN, false);
                            break;
                        case EVENT_RO_SMOKE_BOMB:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_RO_SMOKE_BOMB, false);
                            events.ScheduleEvent(EVENT_RO_SMOKE_BOMB, 15000);
                            break;
                        case EVENT_RO_WOUND_POISON:
                            me->CastSpell(me->getVictim(), SPELL_RO_WOUND_POISON, false);
                        ...
                        case EVENT_SH_CHAIN_LIGHTNING:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_SH_CHAIN_LIGHTNING, false);
                        ...
                        case EVENT_WL_BANE_OF_DOOM:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_WL_BANE_OF_DOOM, false);
                            break;
                        case EVENT_WL_UNSTABLE_AFFLICTION:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_WL_UNSTABLE_AFFLICTION, false);
                            events.ScheduleEvent(EVENT_WL_UNSTABLE_AFFLICTION, 10000);
                            break;
                        case EVENT_WL_RAIN_OF_FIRE:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_WL_RAIN_OF_FIRE, false);
                            events.ScheduleEvent(EVENT_WL_RAIN_OF_FIRE, urand (7000, 10000));
                            break;
                        case EVENT_WR_MORTAL_STRIKE:
                            me->CastSpell(me->getVictim(), SPELL_WR_MORTAL_STRIKE, false);
                            events.ScheduleEvent(EVENT_WR_MORTAL_STRIKE, 10000);
                            break;
                        case EVENT_WR_HEROIC_LEAP:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_WR_HEROIC_LEAP, true);
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_EMPOWERED_SMITE:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_EMPOWERED_SMITE, false);
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_FIREBOLT:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_FIREBOLT, false);
                            events.ScheduleEvent(EVENT_FIREBOLT, 5000);
                            break;
                    }
                }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_VENOM_SPIT:
                            if(Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                                me->CastSpell(target, SPELL_VENOM_SPIT, false);
                            events.ScheduleEvent(EVENT_VENOM_SPIT, 5000);
                            break;
                    }
                }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_MIND_NUMBING_POISON:
                            me->CastSpell(me->getVictim(), SPELL_MIND_NUMBING_POISON, false);
                            break;
                    }
                }
                DoMeleeAttackIfReady();
