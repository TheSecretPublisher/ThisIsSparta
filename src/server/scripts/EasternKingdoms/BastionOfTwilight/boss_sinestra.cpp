                    ...
                    switch (eventId)
                    {
                            ...
                            for (uint8 i = 0; i < 2; i++)
                            {
                                if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0.0f, 100.0f, true, -SPELL_PURPLE_BEAM))
                                {
                                    ...
                                    if (Creature* orb = me->SummonCreature(49863, pos, TEMPSUMMON_TIMED_DESPAWN, 15000, 0))
                                    {
                                        ...
                                        orb->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
                        ...
                        case EVENT_ORB_START_CHANNEL:
                            if (orbs[0] && orbs[1])
                            {
                                ...
                                if (orbs[1]->getVictim())
                                    orbs[1]->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
                                    orbs[1]->GetMotionMaster()->MoveChase(orbs[1]->getVictim());
                                    
                                if (orbs[0]->getVictim())
                                    orbs[0]->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_MOVE);
        ...
        class spell_sinestra_twilight_essence_SpellScript : public SpellScript
        {
            ...
            void Hit(SpellEffIndex effIndex)
            {
                ...
                if (GetHitUnit()->GetEntry() == 48050)
                {
                    ...
                    {
                        if (Creature* sinestra = ObjectAccessor::GetCreature(*GetHitUnit(), instance->GetData64(DATA_SINESTRA)))
                        {
            ...
            void HandlePeriodicTick(AuraEffect const* /*aurEff*/)
            {
                ...
                if (Unit* caster = GetCaster())
                {
                    if (caster->GetHealthPct() <= 1)
                    {
                        ...
                        caster->MonsterYell("All is lost.... Forgive me, my Queen....", LANG_UNIVERSAL, 0);
