...
class boss_shannox : public CreatureScript
{
    public:
        ...
        struct boss_shannoxAI : public BossAI
        {
            ...
            /*void MoveInLineOfSight(Unit* who) //MOVED TO INSTANCE_FIRELANDS.CPP
            {
                if (!introDone && me->IsWithinDistInMap(who, 70.0f))
                {
                    me->MonsterYell(SAY_INTRO, 0, 0);
                    DoPlaySoundToSet(me, SOU_INTRO);
                    introDone = true;
                }
            }*/
            ...
            void JustReachedHome()
            {
                if(pRiplimb == NULL)
                {
                    pRiplimb = me->SummonCreature(NPC_RIPLIMB, 
                                                  me->GetPositionX()-5,
                                                  me->GetPositionY()-5,
                                                  me->GetPositionZ(),
                ...
                if (pRiplimb)
                {
                    ...
                    pRiplimb->GetMotionMaster()->MoveFollow(me, walkRiplimbDistance, walkRiplimbAngle, MOTION_SLOT_ACTIVE);
                }

                if(pRageface == NULL)  
                {
                    pRageface = me->SummonCreature(NPC_RAGEFACE, 
                                                   me->GetPositionX()+5,
                                                   me->GetPositionY()+5,
                                                   me->GetPositionZ(),
                                                   TEMPSUMMON_MANUAL_DESPAWN);
                ...
                if (pRageface)
                {
                    ...
                    pRageface->GetMotionMaster()->MoveFollow(me, walkRagefaceDistance, walkRagefaceAngle, MOTION_SLOT_ACTIVE);
                }
            }
                ...
                if (me->HealthAbovePct(30) && (me->GetMaxHealth() *0.3f) >(me->GetHealth() - damage))
                    {
                        if (pRiplimb && pRiplimb->isAlive())
                            pRiplimb->AddAura(100064,pRiplimb);
                        if (pRageface && pRageface->isAlive())
                            pRageface->AddAura(100064,pRiplimb);
                    }
            ...
            void UpdateAI(uint32 diff)
            {
                ...
                while (uint32 eventid = events.ExecuteEvent())
                {
                    switch (eventid)
                    {
                        ...
                        case EVENT_HURL_SPEAR_OR_MAGMA_RUPTURE:
                            if(pRiplimb && pRiplimb->isDead())
                            { // If riplimb is dead will cast magma rupture
                                DoCastVictim(SPELL_MAGMA_RUPTURE_SHANNOX);
                                events.ScheduleEvent(EVENT_HURL_SPEAR_OR_MAGMA_RUPTURE, urand(20000,25000));
                                if (!hasSpear)
                                    hasSpear = true;
                            }
                            else
                ...
                if (pRiplimb && pRageface && (pRiplimb->isDead() || pRageface -> isDead()) && !softEnrage)
                {
